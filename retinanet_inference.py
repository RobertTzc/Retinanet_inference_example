...
class Retinanet_instance():
    ...
    def load_model(self):
        ...
        if (self.load_w_config):
            ...
            self.encoder = DataEncoder_fusion(anchor_wh=cfg['KNN_anchors'])
    ...
    def inference(self,image_dir,slice_overlap,read_GPS = False):
        ...
        for index, sub_image in enumerate(sub_image_list):
            with torch.no_grad():
                ...
                boxes, labels, scores = self.encoder.decode(
                    loc_preds.data.squeeze(), cls_preds.data.squeeze(), 512, CLS_THRESH = self.conf_threshold,NMS_THRESH = 0.5)
